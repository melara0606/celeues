  ...
  <input type="text" hidden="true" name="path"  id="path" value="{{url('/')}}"><br>
    <div class="panel"  style="border: 1px bold #ccc; box-shadow: 2px 2px #bbb !important">
      <div class="panel-heading bg-mint">
        {{--<h3 class="panel-title">Editar material didactico ({{ $material->codigo }})</h3>--}}
        <h3 class="panel-title "><p align="left" class="text-m text-bold media-heading mar-no text-main bg"> <strong style="font-size: 14px;color:white">EDITAR MATERIAL DIDACTICO  ({{ $material->codigo }})</strong></p></h3>
        ...
        <div class="panel-body" >
          ...
          <fieldset>
            <div class="form-group has-error" ng-class="{ 'has-success': materialForm.idioma.$valid }">
              ...
              <div class="col-lg-7">
                <select ng-model='item.idioma_id' name="idioma" id="idioma" class="form-control" required>
            ...
            <div class="form-group has-error" ng-class="{ 'has-success': materialForm.categoria.$valid }">
              ...
              <div class="col-lg-7">
                <select ng-model='item.categoria_id' name="categoria" id="categoria"  class="form-control" required>
            ...
            <div class="form-group has-error" ng-class="{ 'has-success': materialForm.niveles.$valid }">
              ...
              <div class="col-lg-7">
                <select ng-model='item.nivel_id' name="niveles" id="niveles"  class="form-control" required>
            ...
            <div class="form-group">
              ...
              <div class="col-lg-7 col-lg-offset-3">
                ...
                <div class="checkbox">
                  <input ng-model='cd' id="appendCD" class="magic-checkbox" type="checkbox" name="cd" />
            ...
            <!-- <div class="form-group has-error" ng-class="{ 'has-success': materialForm.fechaAdd.$valid }">
              ...
              <div class="col-lg-7">
                <div class='input-group date' id='datetimepicker'>
                  <input ng-model='fechaAdd' ng-max='maxDate' type='date'
                    class="form-control" name="fechaAdd" id="fechaAdd" required />
          ...
          <fieldset>
            <div class="form-group has-error" ng-class="{ 'has-success': materialForm.observaciones.$valid }">
              ...
              <div class="col-md-9">
                <textarea id="observaciones" rows="4" class="form-control" ng-model='item.observaciones'
        ...
        <div class="panel-footer">
          <div class="row">
            <div class="col-sm-7 col-sm-offset-3">
              <button class="btn btn-primary btn-md" ng-disabled='materialForm.$invalid' type="submit">Editar</button>
...
@section('script')
  ...
  <script type="text/javascript">
    angular.module("materiales", [], function($interpolateProvider) {
      ...
      $.niftyNav('expand'); 
      $scope.cd = Boolean(<?= $material->is_cd ?>);
      $scope.copia  = Boolean(<?= $material->copia ?>);
      $scope.donado = Boolean(<?= $material->is_donado ?>);
      ...
      $scope.send = function($valid) {
        if( $valid ) {
          var data = Object.assign($scope.item, {
            is_cd: $scope.cd, copia: $scope.copia, donado: $scope.donado
          });

          $.ajax({
            data: data,
            type: 'PUT',
            url:  $('#path').val() + "/materiales/"+ data.id,
            success: function(_r) {
              var rs = JSON.parse(_r);
              if(rs.ok) {
                swal(rs.message, {
                  icon: "success"
                }).then(function() {

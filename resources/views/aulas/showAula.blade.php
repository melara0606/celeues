        ...
        <table id="myTable" class="table table-striped row-border" style="border-top: 1px solid #ccc;border-bottom: 1px solid #ccc; ">
          ...
          <tbody>
            ...
            <tr id="{{ $aula->id }}">
              ...
              <td align="Center"><div class="label label-table bg-dark"><div class="text-xs text-bold"></div>{{ $aula->nombre }}</div></td>
              <td >{{ $aula->capacidad }}</td>
              @if($aula->estado=='ACTIVO')
              <td align="center"><div class="label label-table bg-mint"><div class="text-xs text-bold"></div>{{ $aula->estado }}</div></td>
              @endif
              @if($aula->estado=='INACTIVO')
              <td align="center"><div class="label label-table bg-gray"><div class="text-xs text-bold"></div>{{ $aula->estado }}</div></td>
              @endif
              ...
              <td align="center">
                ...
                --}}
                <button class="btn btn-icon btn-default btn-default btn-sm  btn-hover-mint add-tooltip editarmodal" data-original-title="Editar Registro" data-container="body" value="{{ $aula->id }}"><i class="demo-psi-pen-5 icon-sm " ></i> Editar</button>
                <button class="btn btn-icon btn-default btn-sm  btn-hover-info infoModal add-tooltip " data-original-title="Información" data-container="body" value="{{ $aula->id }}"><i class="demo-pli-exclamation icon-sm " ></i> Info</button>
                @if($aula->estado=='ACTIVO')
                <button class="btn btn-icon btn-default btn-default btn-sm  btn-hover-danger darbaja" value="{{ $aula->id }}"><div class="demo-icon"><i class="ion-chevron-down"></i><span> Dar Baja</span></div> </button>
                @endif
                @if($aula->estado=='INACTIVO')
                <button class="btn btn-icon btn-default btn-default btn-sm  btn-hover-primary darAlta" value="{{ $aula->id }}"><div class="demo-icon"><i class="ion-chevron-up"></i><span> Dar Alta</span></div> </button>
                @endif
                {{--<button type="button" class="btn btn-outline-info btn-sm infomodal" value="{{ $aula->id }}">Info</button>--}}
  ...
  <div class="modal-dialog {{--modal-lg--}}">
    <div class="modal-content">
      ...
      <div class="modal-header alert-primary" id="modalIngresoHeader" >
        ...
        <h4 class="modal-title" style="color: white;" id="modalIngresoLabel"><label>Ingresar aulas</label></h4>
      </div>
  ...
  <div class="modal-dialog {{--modal-lg--}}">
    <div class="modal-content">
      <div class="modal-header alert-info">
        <button type="button" class="close" data-dismiss="modal"><i class="pci-cross pci-circle"></i></button>
        <h4 class="modal-title" style="color: white;" id="myLargeModalLabel">Datos de Aula</h4>
      </div>
      <div class="modal-body">
